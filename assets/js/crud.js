...
const CRUD = {
  ...
  WRITE_RULES: {
    ...
    cbt:['staff','teacher'], cbt_prompts:['staff','teacher'], assignments:['staff','teacher','student'],
    ...
    complaints:['staff','teacher','parent','student'], inbox:['staff','teacher','parent','student'],
    ...
    admissions:[], departments:[], front_desk:[], document_builder:[], fleet_tracking:[], facility_booking:[],
  ...
  SCHEMA: {
    ...
    parent_child: { table:'parent_child', title:'Parent–Child Link', cols:[
  ...
  */
  async renderList(moduleId, options = {}) {
    ...
    const studentOwnedModules = ['results', 'assignments', 'attendance', 'fees', 'report_cards', 'certificates', 'messages', 'inbox', 'complaints', 'eresources', 'digital_library', 'library_borrowers'];
    // Modules that parents should see their children's data for
    const parentViewModules = ['results', 'assignments', 'attendance', 'fees', 'report_cards', 'certificates'];
    ...
    if (isStudent && studentOwnedModules.includes(key) && currentUserId) {
      try {
        const { data: st } = await this.sb.from('students').select('id,full_name,class,class_name').eq('user_id', currentUserId).maybeSingle();
        if (st) {
          filteredData = (data || []).filter(r =>
            r.student_id === st.id || r.student_id === currentUserId || r.user_id === currentUserId ||
            r.created_by === currentUserId || r.recipient_id === currentUserId || r.profile_id === currentUserId ||
            (r.student_name && st.full_name && String(r.student_name).toLowerCase() === String(st.full_name).toLowerCase()) ||
            (r.class && (r.class === st.class || r.class === st.class_name))
          );
        }
      } catch(e) { filteredData = data || []; }
    }
